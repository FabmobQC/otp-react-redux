...
import { removeUnusedQueryParams } from '../util/api'

const { filterProfileOptions } = coreUtils.profile
const { getDefaultQuery, getUrlParams } = coreUtils.query
...
export function getInitialState(userDefinedConfig) {
  ...
  removeUnusedQueryParams(defaults)
    ...
    switch (action.type) {
      ...
      */
      ...
      case 'SET_ACTIVE_STEP':
        ...
        return state
      case 'SET_ADDITIONAL_PLACE_WAITING_TIME': {
        return update(state, {
          currentQuery: {
            additionalPlacesWaitingTimes: {
              [action.payload.index]: {
                $set: action.payload?.waitingTime
              }
            }
          }
        })
      }
      case 'CLEAR_ADDITIONAL_PLACE_WAITING_TIME': {
        return update(state, {
          currentQuery: {
            additionalPlacesWaitingTimes: {
              [action.payload.index]: { $set: undefined }
            }
          }
        })
      }
      case 'SET_LOCATION':
        if (action.payload?.locationType?.includes('additional-place')) {
          const index = action.payload.locationType.split('-')[2] ?? 0
          return update(state, {
            currentQuery: {
              additionalPlaces: {
                [index]: { $set: action.payload?.location }
              }
            }
          })
        }
      ...
      case 'CLEAR_LOCATION':
        if (action.payload?.locationType?.includes('additional-place')) {
          ...
          return update(state, {
            currentQuery: {
              additionalPlaces: { $splice: [[index, 1]] }
            }
          })
        }
        return update(state, {
