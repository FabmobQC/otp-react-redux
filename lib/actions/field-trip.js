...
const FieldTripEndPoints = {
  ...
  SET_DESTINATION_ARRIVAL_TIME: 'setRequestArriveDestinationTime',
  SET_DESTINATION_DEPART_TIME: 'setRequestLeaveDestinationTime',
  ...
  SET_SCHOOL_ARRIVAL_TIME: 'setRequestArriveSchoolTime'
}
 ...
 */
function prepareQueryParams(request, outbound, showRePlanPopup) {
  return async function (dispatch, getState) {
    ...
    const confirmLocationOverride =
      showRePlanPopup && hasExistingLocations
        ? window.confirm(
    ...
    const locations = !confirmLocationOverride
...
function getMissingTripHashesForActiveItineraries() {
  return async function (dispatch, getState) {
    ...
    const { tripHashLookup } = state.callTaker.fieldTrip
    ...
    tripHashesToRequest.forEach(async (tripId) => {
      dispatch(
        createGraphQLQueryAction(
          ...
          {},
          (hash) => {
            dispatch(receiveTripHash({ hash, tripId }))
          },
          console.warn,
          {
            noThrottle: true,
            rewritePayload: (payload) =>
              payload?.data?.trip?.semanticHash || null
          }
        )
      )
    })
...
function makeFieldTripPlanRequests(request, outbound, intl) {
  return async function (dispatch, getState) {
    ...
    while (shouldContinueSearching) {
      ...
      dispatch(getMissingTripHashesForActiveItineraries())
...
export function planTrip(request, outbound, intl) {
  return async function (dispatch, getState) {
    ...
    await dispatch(prepareQueryParams(request, outbound, true))
...
export function viewRequestTripItineraries(request, outbound) {
  return async function (dispatch, getState) {
    ...
    await dispatch(prepareQueryParams(request, outbound, false))
...
/**
 * Sets one of the 3 field trip times for the requested field trip.
 */
export function setFieldTripTime(request, time, intl, endpoint) {
  return updateFieldTripRequest(
    request,
    FieldTripEndPoints[endpoint],
    { time },
    intl,
    () =>
      intl.formatMessage({
        id: 'actions.fieldTrip.setTimeError'
      }),
    true
  )
}
