...
export function routingQuery(searchId = null, updateSearchInReducer) {
  ...
  return function (dispatch, getState) {
    ...
    combinations.forEach((combo, index) => {
      ...
      query.variables.additionalPlaces = currentQuery.additionalPlaces.map(
        (place) => place && `${place.name}::${place.lat},${place.lon}` // fabmob: copy pasted from generateOtp2Query in otp-ui
      )
      query.variables.additionalPlacesWaitingTimes =
        currentQuery.additionalPlacesWaitingTimes
