...
import { isGeoJsonFlex } from '@opentripplanner/stops-overlay'
 ...
 */
export function createGraphQLQueryAction(
  query,
  variables,
  responseAction,
  errorAction,
  ...
  return createQueryAction(null, responseAction, errorAction, {
    ...
    fetchOptions,
    noThrottle: true,
    url: '/index/graphql' // fabmob: otp-react-redux changed it to 'gtfs/v1'. Not sure why.
  })
}
    ...
    dispatch(
      createGraphQLQueryAction(
        ...
        receivedStopsWithinBBoxError,
        {
          ...
          noThrottle: true,
          rewritePayload: (payload) => {
            // Flex stops need to be assigned a color, otherwise the rendering in StopsOverlay fails.
            const mappedStops = payload.data.stops.map((s) =>
              isGeoJsonFlex(s.geometries?.geoJson)
                ? {
                    ...s,
                    color: `#${s.routes?.[0]?.color || LIGHT_GRAY}`
                  }
                : s
            )
            return { stops: mappedStops }
          },
          serviceId: 'stops'
        }
      )
    ...
    dispatch(
      createGraphQLQueryAction(
        ...
        findFeedsError,
        {
          noThrottle: true,
          rewritePayload: (payload) => {
            // Fabmob: This condition makes the frontend crash on errors.
            /* if (payload.errors) {
...
export function routingQuery(searchId = null, updateSearchInReducer) {
  ...
  return function (dispatch, getState) {
    ...
    // Retrieve active mode keys from URL parameters or user/config defaults
    ...
    const activeModeKeys =
      ...
      }).modeButtons?.filter((mb) => mb !== null) || defaultModeButtons
    ...
    // Get mode setting values from url, or user saved defaults, or config defaults, or definition defaults
    ...
    // Get the query param strings to save for the rider's search history
    // Use effective modeButtons if URL doesn't have them, to ensure saved searches are consistent
    const rawModeButtonQP =
      urlSearchParams.get('modeButtons') ?? activeModeKeys.join('_')
    ...
    const hasCarMode = baseQuery.modes.some(({mode}) => mode === 'CAR')
    const hasTaxiMode = baseQuery.modes.some(({mode}) => mode === 'TAXI' )
    if (hasTaxiMode && !hasCarMode) {
      baseQuery.modes.push({mode: 'CAR'})
    }
    ...
    if (hasTaxiMode) {
      const carTripIndex = combinations.findIndex((c) => c.modes.some(({mode}) => mode === 'CAR'))
      const carTrip = combinations[carTripIndex]
      if (!hasCarMode) {
        combinations.splice(carTripIndex, 1)
      }
      const taxiTrip = {
        ...carTrip,
        modes: carTrip.modes.map((m) => m.mode === 'CAR' ? {mode: 'TAXI'} : m)
      }
      combinations.push(taxiTrip)
    }
    ...
    combinations.forEach((combo, index) => {
      ...
      // fabmob: Insure our custom settings are not being filtered out
      const customSettings = ['taxiStandard', 'taxiMinivan', 'taxiSpecial', 'nbKmPerYear', 'vehiculeType', 'paidParkingTime']
      combo.modes.forEach(({mode}) => {
        combo.modeSettings.forEach((setting) => {
          if (setting.applicableMode === mode && customSettings.includes(setting.key)) {
            query.variables[setting.key] = setting.value
          }
        })
      })
      query.variables.additionalPlaces = []
      query.variables.additionalPlacesWaitingTimes = []
      currentQuery.additionalPlaces.forEach((place, index) => {
        if (place !== undefined && place !== null) {
          query.variables.additionalPlaces.push(
            `${place.name}::${place.lat},${place.lon}`
          ) // fabmob: copy pasted from generateOtp2Query in otp-ui
          query.variables.additionalPlacesWaitingTimes.push(
            currentQuery.additionalPlacesWaitingTimes[index]
          )
        }
      })
      // fabmob: Otherwise the query fails and the server return "Field 'numberOfTransfers' in type 'Itinerary' is undefined"
      // We don't know why this happens.
      query.query = query.query.replace('transfers: numberOfTransfers\n', '')
      dispatch(
        createGraphQLQueryAction(
          ...
          },
          {
            ...
            rewritePayload: (response, dispatch, getState) => {
              ...
              // If validModeCombinations are present, only return itineraries that contain at least one explicitly requested mode,
              // then filter out itineraries that do not match our list of valid mode combinations
              // (e.g. "WALK" + "DRIVE")
