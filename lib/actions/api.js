...
function executeOTPAction(methodName, ...params) {
  return function (dispatch) {
    return dispatch(v2Actions[methodName](...params))
  }
}
 ...
 * Also persists any missing mode defaults (modeButtons and mode settings) into the URL
 * to ensure the planned trip stays stable across auth state changes (sign-in/sign-out).
 */
export function updateOtpUrlParams(state, searchId) {
  ...
  otpParams.additionalPlaces = []
  otpParams.additionalPlacesWaitingTimes = []
  currentQuery.additionalPlaces.forEach((place, index) => {
    if (place !== undefined && place !== null) {
      otpParams.additionalPlaces.push(
        `${place.name}::${place.lat},${place.lon}`
      ) // fabmob: copy pasted from generateOtp2Query in otp-ui
      otpParams.additionalPlacesWaitingTimes.push(
  ...
  return function (dispatch, getState) {
    ...
    // Compute missing mode defaults to persist in URL
    // This ensures the planned trip stays stable if user signs in/out
    const modeDefaultsForUrl = {}

    // Mode buttons: if URL doesn't have them, use the effective defaults
    if (!urlSearchParams.get('modeButtons')) {
      const defaultModeButtons = getDefaultModeButtons(state)
      if (defaultModeButtons && defaultModeButtons.length > 0) {
        modeDefaultsForUrl.modeButtons = defaultModeButtons.join('_')
      }
    }

    // Mode settings: for each setting, if URL doesn't have it, use the effective value
    if (modeSettingDefinitions && modeSettingDefinitions.length > 0) {
      const defaultModeSettingValues = getDefaultModeSettingValues(state)
      const effectiveModeSettingValues = generateModeSettingValues(
        urlSearchParams,
        modeSettingDefinitions,
        defaultModeSettingValues
      )

      for (const setting of modeSettingDefinitions) {
        if (!urlSearchParams.has(setting.key)) {
          const value = effectiveModeSettingValues[setting.key]
          // Skip undefined, null, and empty string but keep false and 0
          if (value !== undefined && value !== null && value !== '') {
            modeDefaultsForUrl[setting.key] = value
          }
        }
      }
    }

    // Merge in the provided OTP params and mode defaults, then update the URL.
    dispatch(setUrlSearch(Object.assign(params, otpParams, modeDefaultsForUrl)))
  }
}
