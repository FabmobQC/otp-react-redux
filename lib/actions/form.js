...
import { planParamsToQueryAsync } from '../../fabmob/@opentripplanner/core-utils/lib/query'
...
const {
  ...
  getUrlParams,
  // planParamsToQueryAsync // Fabmob: we use our own implementation
} = coreUtils.query

export const settingQueryParam = createAction('SET_QUERY_PARAM')
export const clearingActiveSearch = createAction('CLEAR_ACTIVE_SEARCH')
export const setActiveSearch = createAction('SET_ACTIVE_SEARCH')

export function clearActiveSearch() {
  return function (dispatch) {
    dispatch(clearingActiveSearch())

    // Remove the active search from the urlParams
    const urlParams = coreUtils.query.getUrlParams()
    urlParams.ui_activeSearch = undefined
    dispatch(setUrlSearch(urlParams))

    /* There's no need for an active itinerary if there's no active 
    search (and leaving one in the urlParams causes problems */
    dispatch(setActiveItinerary({ index: -1 }))
  }
}
...
export function resetForm(full = false) {
  return function (dispatch, getState) {
    ...
    const { transitModes } = config.modes
    ...
    if (localUserDefaults) {
      ...
      const defaultQuery = Object.assign(getDefaultQuery(config), userOverrides)
...
export function parseUrlQueryString(params = getUrlParams(), source = null) {
  return function (dispatch, getState) {
    const state = getState()
    // Do process the URL and fetch itineraries when refreshing a page showing the trip viewer,
    // in addition to pages showing itinerary results.
    // TODO: Improve the logic to trigger routing when loading from URL.
    if (
      state.otp.ui.mainPanelContent !== null &&
      state.otp.ui.mainPanelContent !== MainPanelContent.TRIP_VIEWER
    ) {
      return state
    }
    ...
    planParamsToQueryAsync(planParams, state.otp.config).then((query) =>
...
export function checkShouldReplanTrip(autoPlan, isMobile, oldQuery, newQuery) { // Determine if either from/to location has changed
  ...
  const oneLocationChanged =
    ...
    (!fromChanged && toChanged) ||
    oldQuery.additionalPlaces.length !== newQuery.additionalPlaces.length ||
    oldQuery.additionalPlaces.some(
      (place, index) => !isEqual(place, newQuery.additionalPlaces[index])
    )
  ...
  const locationChangedToBlank =
    ...
    oldQuery.additionalPlaces.some(
      (place, index) => place && !newQuery.additionalPlaces[index]
    )
  // Fabmob: We replan if all fields are set and at least one has changed
  const shouldReplanTrip =
    oneLocationChanged &&
    newQuery.from &&
    newQuery.to &&
    newQuery.additionalPlaces.every((place) => place)
...
export function switchLocations(index) {
  return async function (dispatch, getState) {
    ...
    let location1 = to
    let location2 = from
    let locationType1 = 'from'
    let locationType2 = 'to'

    if (index !== undefined) {
      location1 = state.otp.currentQuery.additionalPlaces[index]
      location2 =
        index === 0 ? to : state.otp.currentQuery.additionalPlaces[index - 1]
      locationType1 = index === 0 ? 'to' : `additional-place-${index - 1}`
      locationType2 = `additional-place-${index}`
    }
