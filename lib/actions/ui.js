...
import { clearViewedRoutes } from '../../fabmob/actions/fabmob'
import {
  getConfigLocales,
  getDefaultLocale,
...
import { userExistsInDatabase } from '../util/user'
  ...
  return async function (dispatch, getState) {
    ...
    switch (root) {
      ...
      case 'touristic-places':
        dispatch(setMainPanelContent(MainPanelContent.TOURISTIC_PLACES))
        break
      case 'amenities':
        dispatch(setMainPanelContent(MainPanelContent.AMENITIES_VIEW))
        break
 ...
 * If the locale is updated, reload the page to apply the new locale.
 * @param {string} locale The locale to set, e.g., 'en-US'.
 * @param {boolean} isUpdatingLocale If true, reload the page to apply the new locale.
 *                                   This is used when the locale is updated
 *                                   from the locale selector. This prevents
 *                                   an infinite loop of locale updates
 *                                   when the locale is set from the initial
 *                                   page load.
 */
export function setLocale(locale, isUpdatingLocale = false) {
  return async function (dispatch, getState) {
    ...
    if (matchedLocale !== ui.locale) {
      ...
      if (userExistsInDatabase(loggedInUser)) {
