...
export async function loadLocaleData(matchedLocale, customMessages) {
  ...
  // Unless noted, non-English-US translations are not specific to a region.
  switch (matchedLocale) {
    ...
    case 'zh':
    case 'zh-Hans':
      messages = await import('../../i18n/zh_Hans.yml')
      otpUiLocale = 'zh_Hans'
      break
    case 'zh-Hant':
      messages = await import('../../i18n/zh_Hant.yml')
      otpUiLocale = 'zh_Hant'
      break
    case 'ru':
...
export function getFormattedMode(mode, intl) {
  switch (mode?.toLowerCase()) {
    ...
    case 'taxi':
      return intl.formatMessage({ id: 'common.modes.taxi' })
    default:
