...
export function hasValidLocation(query, locationKey) {
  if (locationKey.includes('additional-place')) {
    const index = locationKey.split('-')[2]
    const additionalPlace = query.additionalPlaces[index]
    return additionalPlace?.lat && additionalPlace?.lon
  }
