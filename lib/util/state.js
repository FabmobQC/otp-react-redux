...
export function getFare(itinerary, defaultFareType) {
  ...
  const taxiPricing = itinerary.taxiPricing
  const taxiCost = taxiPricing?.pricing.parts[0].amount
  const taxiCurrency = taxiPricing?.pricing.parts[0].currencyCode

  const carCost = itinerary.drivingCosts
