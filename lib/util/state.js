...
export function getFare(itinerary, defaultFareType) {
  ...
  const taxiCost = (itinerary?.fare?.fare?.regular?.cents ?? 0) / 100
  const taxiCurrency = itinerary?.fare?.fare?.regular?.currency?.currencyCode

  const carCost = itinerary?.legs?.reduce((acc, leg) => {
    if (leg.rideHailingEstimate?.provider?.id === '') {
      return acc + leg.rideHailingEstimate.minPrice.amount
    }
    return acc
  }, 0)
  const carCurrency = itinerary?.legs?.[0]?.rideHailingEstimate?.minPrice?.currency?.code
  ...
  return {
    ...
    maxTNCFare,
    minTNCFare,
    transitFare: taxiCost || (carCost !== 0 && carCost.toFixed(2)) || itineraryCost?.amount
  }
}
  ...
  (
    ...
    if (hasResponse) {
      if (hasItineraryResponse) {
        if (itineraryToRender) {
          // FabMob: "Clone" itinerary and replace 'TAXI' mode by 'CAR' mode
          // This is required since transition.js does not support 'TAXI' mode
          const newItineraryToRender = {
            ...itineraryToRender,
            legs: itineraryToRender.legs.map((leg) => ({
              ...leg,
              mode: leg.mode === 'TAXI' ? 'CAR' : leg.mode
            }))
          }
          return itineraryToTransitive(newItineraryToRender, {
            companies,
            disableFlexArc,
            getTransitiveRouteLabel,
            intl
