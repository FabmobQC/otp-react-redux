...
const MOBILE_CURRENT_POSITION_REFRESH_INTERVAL = 30000
...
class ResponsiveWebapp extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      mobileCurrentPositionRefreshIntervalId: undefined
    }
  }
  ...
  componentDidUpdate(prevProps) {
    ...
    const places = [
      query.from,
      query.to,
      ...(query.additionalPlaces ?? [])
    ].filter((position) => position && position.lat && position.lon)
    ...
    if (currentPosition !== prevProps.currentPosition) {
      ...
      // Fabmob: Find the bounds of the smallest square containing all places
      const bounds = places.slice(1).reduce(
        (acc, current) => {
          const [min, max] = acc
          return [
            {
              lat: Math.min(min.lat, current.lat),
              lon: Math.min(min.lon, current.lon)
            },
            {
              lat: Math.max(max.lat, current.lat),
              lon: Math.max(max.lon, current.lon)
            }
          ]
        },
        [places[0], places[0]]
      ...
      try {
        ...
        console.warn(
          `Couldn't center map around coords, coords likely invalid: ${query}`
        )
      }
    }
    ...
    // While the location field is focused on mobile, clear the automatic position
    // refresh interval. This prevents the page from reloading during typing
    if (
      isMobile() &&
      this.state.locationFieldFocused &&
      this.state.mobileCurrentPositionRefreshIntervalId
    ) {
      clearInterval(this.state.mobileCurrentPositionRefreshIntervalId)
      this.setState({ mobileCurrentPositionRefreshIntervalId: undefined })
    }

    // Restart the automatic position refresh interval if location field is blurred
    if (
      isMobile() &&
      !this.state.locationFieldFocused &&
      this.state.mobileCurrentPositionRefreshIntervalId === undefined
    ) {
      const intervalId = setInterval(() => {
        getCurrentPosition(intl)
      }, MOBILE_CURRENT_POSITION_REFRESH_INTERVAL)
      this.setState({ mobileCurrentPositionRefreshIntervalId: intervalId })
    }
  }
  ...
  componentWillUnmount() {
    ...
    // Clear current position refresh interval
    if (this.state.mobileCurrentPositionRefreshIntervalId !== undefined)
      clearInterval(this.state.mobileCurrentPositionRefreshIntervalId)
  }
  ...
  renderMobileView = () => {
    ...
    const handleLocationFieldFocus = () => {
      this.setState({ locationFieldFocused: true })
    }

    const handleLocationFieldBlur = () => {
      this.setState({ locationFieldFocused: false })
    }
    ...
    return (
      <>
        <PopupWrapper content={popupContent} hideModal={this._hidePopup} />
        <MobileMain
          onLocationFieldBlur={handleLocationFieldBlur}
          onLocationFieldFocus={handleLocationFieldFocus}
        />
      </>
    )
  }
            ...
            >
              <QueryParamProvider adapter={ReactRouter5Adapter}>
                <Switch>
                  ...
                  {extraViews.map((view, index) => {
                    return (
                      <Route
                        component={frame(view.content)}
                        key={`extra-${index}`}
                        path={view.path}
