...
class NarrativeItineraries extends Component {
  ...
  _getCarItin = () => {
    ...
    const isCarOnly = (itin) =>
      itin.legs.length === 1 &&
      (itin.legs[0].mode.startsWith('CAR') ||
        itin.legs[0].mode.startsWith('TAXI'))
    return (
  ...
  render() {
    ...
    const groupedMergedItineraries = itinerariesWithCo2.reduce(
      (prev, cur) => {
        ...
        // Fabmob: We handle "multi" and "single" the same for simplicity
        const modeContainer = modeItinMap.multi
