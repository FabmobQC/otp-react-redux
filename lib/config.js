...
import TaxiIcon from '../fabmob/icons/aiga_taxi.svg'
...
const LegIcon = ({ ...props }) => {
  // eslint-disable-next-line react/prop-types -- Can't even set prop types on a js file
  if (props.leg.mode === 'TAXI') {
    return <img alt="Taxi" src={TaxiIcon} style={{ height: 27, width: 27 }} />
  }
  return <ClassicLegIcon {...props} />
}
const ModeIcon = ({ ...props }) => {
  // eslint-disable-next-line react/prop-types -- Can't even set prop types on a js file
  if (props.mode === 'taxi') {
    return <img alt="Taxi" src={TaxiIcon} style={{ height: 32, width: 32 }} />
  }
  return <ClassicModeIcon {...props} />
}
/**
